...
public:
    ...
    MatrixUDG(char vexs[], int vlen, int matrix[][6]);
    ~MatrixUDG();
 ...
 *     matrix-- 矩阵(数据)
 */
MatrixUDG::MatrixUDG(char vexs[], int vlen, int matrix[][6])
{
    int i, j;
 ...
 */
void MatrixUDG::floyd(int dist[][MAX])
{
    int i, j, k;
    int tmp;
    ...
    for (k = 0; k < mVexNum; k++)
    {
        for (i = 0; i < mVexNum; i++)
        {
            for (j = i + 1; j < mVexNum; j++)
            {
                ...
                if (dist[i][j] > tmp)
                {
                    // "i到j最短路径"对应的值设，为更小的一个(即经过k),由于是无向图，所以可以直接对称赋值
                    dist[i][j] = tmp;
                    dist[j][i] = tmp;
                }
            }
